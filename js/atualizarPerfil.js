async function atualizarPerfil() {
    ...
    try {
        const respostaCadastro = await fetch(`http://3.90.20.205:8080/api/usuarios/${retornaNivelRequisicao(Number(sessionStorage.getItem('nivel_acesso_cod')))}/${sessionStorage.getItem('id')}`, {
            method: "PUT",
...
async function atualizarMeta() {
    ...
    if (meta != sessionStorage.getItem("meta")) {
        const respostaAtt = await fetch(`http://3.90.20.205:8080/api/metas/${sessionStorage.getItem("id")}`, {
            method: "PUT",
            body: meta,
            headers: { 'Authorization': `Bearer ${token}`, "Content-type": "application/json; charset=UTF-8" }
        });
...
async function atualizaHorarioAtendimento() {
    ...
    if (validacaoUm || validacaoDois || validacaoTres || validacaoQuatro) {
        ...
        const respostaAtt = await fetch(`http://3.90.20.205:8080/api/horario-professor/${id}`, {
            method: "PUT",
            body: JSON.stringify(dados),
            headers: { 'Authorization': `Bearer ${token}`, "Content-type": "application/json; charset=UTF-8" }
        });
...
async function atualizarNivelIngles() {
    ...
    if (novosnivel.length > 0) {
        for (const nivelIngles of novosnivel) {
            ...
            try {
                const respostaCadastro = await fetch(`http://3.90.20.205:8080/api/usuario-nivel-ingles`, {
                    method: "POST",
...
async function atualizarItensUsuario(prefixo, todosItens, selectedSet) {
    ...
    try {
        const url = `http://3.90.20.205:8080/api/usuario-${prefixo === 'nivel' ? 'nivel-ingles' : 'nicho'}/professor/${sessionStorage.getItem("id")}`;
        console.log(dados)
