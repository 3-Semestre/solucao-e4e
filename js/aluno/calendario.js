...
async function buscarProfessores() {
  try {
    const response = await fetch('http://3.90.20.205:8080/api/usuarios/professor', {
      method: 'GET',
      headers: {
...
professorSelect.addEventListener('change', async (event) => {
  ...
  console.log(data)
  try {
    const response = await fetch(`http://3.90.20.205:8080/api/horario-professor/disponiveis/${selectedProfessor}?dia=${data}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
...
async function salvarAgendamento(professorId, horario) {
  ...
  try {
    const respostaAgendamento = await fetch("http://3.90.20.205:8080/api/agendamento", {
      method: "POST",
...
async function cancelarAgendamento(id) {
  try {
    const respostaAgendamento = await fetch(`http://3.90.20.205:8080/api/agendamento/${id}`, {
      method: 'GET',
      headers: {
    ...
    const respostaStatus = await fetch(`http://3.90.20.205:8080/api/status/4`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    const novoStatus = await fetch("http://3.90.20.205:8080/api/historico-agendamento", {
      method: "POST",
...
async function carregarEventos() {
  try {
    ...
    const response = await fetch(`http://3.90.20.205:8080/api/agendamento/1/${sessionStorage.getItem('id')}/${currentMonth}/${currentYear}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
